...
async def test_extract_pdf_and_preview(tmp_path):
    ...
    with patch("app.services.ingest.PdfReader", return_value=Reader()):
        assert "hello page" in ingest.extract_pdf_text(bad)
...
@pytest.mark.asyncio
async def test_resolve_raw_text_branches(tmp_path):
    assert await ingest.resolve_raw_text(PendingPayload(source_type="text", text="hi"), None) == "hi"
    assert "without text" in await ingest.resolve_raw_text(PendingPayload(source_type="text"), None)
    ...
    with (
        patch("app.services.ingest.extract_pdf_text", return_value="short"),
        patch("app.services.ingest.render_pdf_preview", return_value=None),
    ):
        text = await ingest.resolve_raw_text(PendingPayload(source_type="document"), pdf)
        assert "short" in text or "document" in text

    with patch("app.services.ingest.extract_pdf_text", return_value="long pdf text " * 10):
        text = await ingest.resolve_raw_text(PendingPayload(source_type="document"), pdf)
        assert "long pdf text" in text
    ...
    text = await ingest.resolve_raw_text(
        PendingPayload(source_type="forward", caption="cap"),
        other,
    )
    assert text == "cap"
...
async def test_rag_answer_paths(settings):
    ...
    client.chat.completions.create = AsyncMock(
        side_effect=[
            ...
            make_chat_response('{"rewritten_query":"oil"}'),
            make_chat_response("answer"),
            make_chat_response("{}"),
        ]
    )
    ...
    with patch("app.db.repository.search_similar", AsyncMock(return_value=[item])):
        answer, items = await rag.answer_question(session, "oil again", client=client, settings=settings)
        assert items == [item]

    with patch("app.db.repository.search_similar", AsyncMock(return_value=[])):
        answer, items = await rag.answer_question(session, "missing", client=client, settings=settings)
        assert items == []
